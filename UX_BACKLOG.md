...
| 25 | Now Playing viste idle-melding selv når Spotify spilte musikk. Rotårsak: `/api/now` lente seg eksklusivt på tracker-tråden som skriver til `now_playing`-tabellen hvert 7. s. Tråden avsluttes permanent ved Spotify 401 (token-feil under Railway-omstart) og restarter ikke automatisk. Staleness-terskel var 20 s (bare ~2,9 poll-sykluser margin). Løst med to-trinns strategi: (1) servér fersk cache (≤ 30 s) direkte; (2) ved manglende/stale cache, spør Spotify `/v1/me/player` direkte med brukerens token. Tracker-feiler bryter ikke lenger NowPlaying. | `519439c` |
